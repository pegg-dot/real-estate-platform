-- Migration 0015 — exit-strategy optimizer output on property_score (spec 019 Part A).
-- Persist the recommended exit strategy + the ranked/excluded menu so the Deal Genome and the
-- dossier can surface "the highest, legal, thesis-fit way to run this house."

begin;

alter table property_score
  add column if not exists recommended_exit_strategy text,                      -- ranked #1 use
  add column if not exists exit_strategies          jsonb not null default '[]'::jsonb; -- the menu

-- recreate the genome view to expose the new fields (drop+create: the new cols sit mid-list,
-- and CREATE OR REPLACE can't reorder existing view columns)
