...
export default function PipelineBoard({ deals }: { deals: PipelineDeal[] }) {
  ...
  return <div className="page wide pipeline-page">
    ...
    <div className="pipeline-controls"><label className="explorer-search pipeline-search"><Icon name="search" size={15} /><input type="search" aria-label="Search pipeline" placeholder="Find a deal..." value={search} onChange={event => setSearch(event.target.value)} /></label><label className="archive-control"><input type="checkbox" checked={archive} onChange={event => setArchive(event.target.checked)} />Include exited &amp; passed</label></div>
    ...
    <div className="board" aria-label="Deal stages">{STAGES.filter(([key]) => archive || !["exited", "passed"].includes(key)).map(([stage, label]) => { const items = shown.filter(d => d.stage === stage); return <section key={stage} className="col" aria-label={label}><div className="col-head"><span className={`stage-dot stage-${stage}`} /><h2 className="eyebrow">{label}</h2><span className="stage-count">{items.length}</span></div><div className="col-body">{items.map(deal => <DealCard key={deal.id} dealId={deal.id} apn={deal.apn} stage={deal.stage} address={deal.address} score={deal.score} structure={deal.recommended_structure} />)}{!items.length && <p className="col-empty">No deals in this stage</p>}</div></section>; })}</div>
