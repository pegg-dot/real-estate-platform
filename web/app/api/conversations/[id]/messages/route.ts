...
export async function POST(req: Request, { params }: RouteContext) {
  ...
  const [owned] = await sql()<Array<{ id: string }>>`select id from conversation where id = ${id} and user_id = ${uid}`;
  ...
  await sql()`
    ...
    values (${id}, ${role}, ${agent}, ${content}, ${context}::jsonb, ${trace}::jsonb, ${proposals}::jsonb)`;
