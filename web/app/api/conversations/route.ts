...
// Scoped to the current user (the legacy user when auth is off → unchanged single-user behavior).
export async function GET() {
  ...
  const rows = await sql()<Array<{ id: string; title: string; agent: string; updated_at: string }>>`
    ...
    from conversation where user_id = ${uid} order by updated_at desc limit 200`;
...
export async function POST(req: Request) {
  ...
  const [row] = await sql()<Array<{ id: string; title: string; agent: string }>>`
    insert into conversation (agent, user_id) values (${agent}, ${uid}) returning id, title, agent`;
