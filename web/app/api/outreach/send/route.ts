...
export async function POST(req: Request) {
  ...
  const [draft] = await sql()<Array<{ to_addr: string | null; subject: string; body: string; status: string }>>`
    select to_addr, subject, body, status from email_draft where id = ${id}`;
  ...
  try {
    ...
    await sql()`update email_draft set status = 'sent', detail = coalesce(detail, '{}'::jsonb) || ${sql().json({ gmailId, sentBy: userId })} where id = ${id}`;
