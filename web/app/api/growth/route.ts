...
export async function GET() {
  const areas = await sql()<Array<{ area_key: string; corridor_score: number; value_slope: string | null; median_value: string | null; parcels: number }>>`
    select area_key, corridor_score, value_slope, median_value, parcels
    from growth_area
  ...
  return Response.json({ cells }, { headers: { "cache-control": "no-store" } });
}
