...
def run_checks(page, fixture, errors):
    ...
    expect(page.get_by_role("button", name=re.compile(r"^Re-score properties"))).to_be_visible()
    assert page.evaluate("sessionStorage.getItem('lot_run_pc')") is None
    page.get_by_role("button", name=re.compile(r"^Re-score properties")).click()
    ...
    if os.environ.get("LOT_UI_HARNESS") == "true":
        ...
        lead.get_by_role("button", name="Draft", exact=False).click()
        expect(lead.get_by_text("Action not confirmed.", exact=False)).to_be_visible()
        expect(lead.get_by_role("button", name="Draft", exact=False)).to_be_enabled()
        fixture.action_failure = False
        before = len([call for call in fixture.calls if call[0] == "/api/actions"])
        page.once("dialog", lambda dialog: dialog.dismiss())
        lead.get_by_role("button", name="Record reply", exact=True).click()
        assert len([call for call in fixture.calls if call[0] == "/api/actions"]) == before
    fixture.empty = True
    page.goto(BASE, wait_until="networkidle")
    ...
    if os.environ.get("LOT_UI_HARNESS") == "true":
        for route, filename in [("/ui-qa/pipeline", "pipeline-mobile.png"), ("/ui-qa/leads", "leads-mobile.png"), ("/settings", "settings-mobile.png")]:
            ...
            assert page.evaluate("document.documentElement.scrollWidth <= window.innerWidth"), route + " overflows mobile viewport"
...
if __name__ == "__main__":
    ...
    with sync_playwright() as pw:
        ...
        try:
            run_checks(page, fixture, errors)
        except Exception:
