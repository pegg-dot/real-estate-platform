...
- **HBU IRR is now live + correct.** A Postgres numeric→string concat bug had floored develop
  returns at −99%; fixed + the market re-scored. The deal panel now shows IRR, carry, capital-tied-up
  months, excluded reasons, and the honesty note.
- **Gmail/Calendar connectors + multi-user accounts are BUILT and flag-gated OFF** — today the app
  runs exactly as before (single-user; drafts/events stay in-app). To turn them on, do the operator
  steps below (cloud config + env you control; no code).

Still to look at (not launch-blocking):
- **Pre-existing data-quality item:** ~26 parcels assessed at ~$100 (vacant slivers / common-area
  artifacts) show wild negative hold cash-on-cash (e.g. −5700%/yr) because fixed costs exceed
  near-zero modeled rent. Consider gating parcels below a price floor out of the scored map, or
  clamping the displayed hold CoC. Orthogonal to the IRR work.

### Operator setup — turn on connectors + multi-user (only you can do these)

> ⚠️ **Rotate first.** The Google **client secret** was pasted in chat, so treat it as compromised.
> Google Cloud Console → APIs & Services → Credentials → your OAuth client → **Reset secret**, then
> put the new value in `.env` (`GOOGLE_CLIENT_SECRET`). The client *ID* is fine.
...
2. OAuth consent screen: External, app name "LOT", your support email. Add scopes: `openid`, `email`,
   `profile`, `gmail.send`, `calendar.events`. Add **you + your brother as Test users** (Testing mode
   is fine for 2 users — no Google verification needed).
3. Credentials → your OAuth 2.0 Web client → add **Authorized redirect URIs** (real domain + localhost):
...
| `CONNECTOR_SECRET` | seals stored Gmail/Calendar tokens (AES-256-GCM) | connectors *(already set — random 32-byte)* |
| `AUTH_SECRET` | signs the login session cookie (HMAC) | multi-user auth *(already set — random 32-byte)* |
...
**C. Turn it on**
- **Connectors only** (single-user, just want Gmail/Calendar for you): set `GOOGLE_*` +
  `CONNECTOR_SECRET`, leave `AUTH_ENABLED` unset. Settings → Connections → **Connect**, grant
...
**D. Deploy target** — a **long-running Node host** (Railway / Render / Fly / a VM), **not Vercel
serverless**. The chat streaming, engine bridge (`tsx` child processes), Python ingestion, and the
self-running refresh all need a persistent process + filesystem. Set all env vars there.

**E. Autonomous weekly refresh** (optional) — it's a GitHub Action
(`.github/workflows/weekly-refresh.yml`) gated on a repo secret. To enable: GitHub → repo → Settings
→ Secrets → Actions → add `SUPABASE_DB_URL`. Until then it no-ops (and the in-app "self-running"
refresh only fires when someone opens the homepage on the persistent host).

**What works the instant you finish A–C:** agents draft owner emails → you click **Send** → it goes
